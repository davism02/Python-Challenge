...
Month_ChangeAvg = sum(Month_Change) / len(Month_Change)
...
Month_ChangeList = [i for i in zip(MnthList[1:], Month_Change)]

for row in Month_ChangeList:
...
print(f"Average  Change: ${round(Month_ChangeAvg, 2)}")
print(f"Greatest Increase in Profits: {MxMnth} (${max(Month_Change)})")
print(f"Greatest Decrease in Profits: {MnMnth} (${min(Month_Change)})")
...
with open("FinancialAnalysis.txt", "a") as txt:
    ...
    txt.write(f"Average  Change: ${round(Month_ChangeAvg, 2)}/n")
    txt.write(f"Greatest Increase in Profits: {MxMnth} (${max(Month_Change)})/n")
    txt.write(f"Greatest Decrease in Profits: {MnMnth} (${min(Month_Change)})/n")
    txt.close()